...
export class TerrainComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Listen to shared Terrain/Plant panel state
    this.panelSub = this.mapService.panelOpen$.subscribe(panel => {
      this.menuHidden = panel !== 'none';
  ...
  togglePanel(): void {
    ...
    if (this.panelOpen) {
      this.mapService.setPanelOpen('terrain');
    } else {
      this.mapService.setPanelOpen('none');
      this.cancelPick();
    }
  ...
  togglePick(target: PickTarget): void {
    ...
    this.pickSub = this.mapService.coordPicked$
      .pipe(take(1))
      .subscribe(coord => {
  ...
  generate(): void {
    ...
    this.terrainService
      ...
      .subscribe({
        ...
        error: err =>
          this.handleError('Execute failed', err),
      });
  }

  // ── Poll job status ────────────────────────────────────────
  private startPolling(): void {
    this.pollSub = this.terrainService
      .pollStatus(this.jobId, 2000)
      .subscribe({
      ...
      .subscribe({
        ...
        next: res => {
          this.ngZone.run(() => {
            ...
            const url =
              this.terrainService.getPngUrl(
                res.fileName
              );

            this.mapService.showImageOverlay(
              url,
              res.extent
            );
