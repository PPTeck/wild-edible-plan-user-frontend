...
export class PlantComponent implements OnInit, OnDestroy {
  ...
  get canDownload(): boolean {
    return this.sessionService.canDownload();
  }

  ngOnInit(): void {
    this.fetchPlants();

    // Listen to Terrain/Plant shared panel state
  ...
  togglePanel(): void {
    ...
    this.mapService.setPanelOpen(
      this.panelOpen ? 'plant' : 'none'
    );
  ...
  downloadPDF(): void {
    ...
    doc.text(
      `Total Records: ${this.plants.length}`,
      14,
      26
    );
